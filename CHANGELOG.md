...
### Added
- `database.trino.timeout` (optional, default: 300s/5min): `TrinoProvider.execute_query()` now enforces a query timeout — a watchdog cancels the query server-side (`cursor.cancel()` → Trino coordinator DELETE) instead of letting it run unbounded, and raises `TimeoutError` to the caller. Mirrors `BigQueryProvider`'s existing `timeout` config. Fixes MCP (`ask_database`) queries observed running 10+ minutes with no way to kill them; applies to every path that calls `execute_query` (`/query`, `/auth/query`, dashboards, scheduled queries, MCP).
- `database.postgresql.timeout` / `database.mysql.timeout` (optional, default: 300s/5min): same watchdog-timeout protection extended to `PostgreSQLProvider` and `MySQLProvider`, closing the parity gap left by the Trino/BigQuery-only fix above. Postgres cancels via `conn.cancel()` (psycopg2's real `PQcancel`); MySQL has no client-side cancel, so its watchdog opens a throwaway second connection and runs `KILL QUERY <connection_id>`. Both raise `TimeoutError` to the caller on abort.
