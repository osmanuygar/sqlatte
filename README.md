...
<p align="center">
  <strong>Enterprise AI Data Gateway</strong><br>
  Secure, audited, governed access to your data warehouse — for analysts via natural language, and for AI agents via MCP.
</p>
...
SQLatte is an **Enterprise AI Data Gateway** — a secure, governed layer between AI and your data warehouse. It serves two audiences at once: analysts who want to ask questions in plain English, and AI agents (Claude Desktop, Claude Code, any MCP client) that need controlled, audited access to data without holding raw credentials.

Every query — whether typed by a human or generated by an AI agent — passes through the same validation, access control, and audit pipeline.
...
Most teams give AI assistants raw database credentials. SQLatte sits in between: the AI agent gets a short-lived token tied to specific catalogs and schemas, every generated query is validated before execution, and every call is logged. Revoking an agent's access is one click.

Use SQLatte from Claude Desktop, Claude Code, or any MCP-compatible client:
